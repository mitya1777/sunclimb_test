...
static void LCD_depiction(uint8_t info_to_output)	{
	...
	switch (info_to_output)	{
		case PWM:
			...
			BSP_LCD_GLASS_DisplayChar((uint8_t *) ((uint32_t) &letters[2]), POINT_OFF, DOUBLEPOINT_ON, LCD_DIGIT_POSITION_3);
		...
		case U_IN:
			...
			BSP_LCD_GLASS_DisplayChar((uint8_t *) ((uint32_t) &letters[8]), POINT_OFF, DOUBLEPOINT_ON, LCD_DIGIT_POSITION_3);
...
void interrupts_handling(uint8_t intrpt_setting)
{
...
static void DMA_init(void)	{
	...
	DMA1_CSELR -> CSELR |= (0x00 << DMA_CSELR_C1S_Pos);
